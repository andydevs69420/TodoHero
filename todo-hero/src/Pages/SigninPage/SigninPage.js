...
const SigninPage = () => {
    ...
    (function RenderGreet() {
        ...
        /** greeting delay */
        const greet = "Welcome to TodoHero!";
        
        useEffect(() => {
            for(let idx = 0; idx < greet.length; idx++) 
            { 
                setTimeout(function() {
                    return onStateUpdate((oldValue) => ({
                        ...oldValue,
                        greet: oldValue.greet + greet[idx]
                    }));
                }, (idx * 100)); 
            }
        }, [/**/]);
    ...
    return (
        <section id="signin-page" className="d-block position-relative p-0 py-sm-5 w-100 h-100 bg-primary">
            ...
            <div id="signin-page__bg" className="d-inline-block position-absolute">
                ...
                <h2 className="fs-1 fw-bold text-light text-center">{state.greet}</h2>
            </div>
